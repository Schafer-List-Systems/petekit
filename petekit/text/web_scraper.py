...
class WebScraper(BufferManager, AgenticObject):
    ...
    Workflow:
      ...
      3. web_scrape(url, ...) / web_select_css(url, selector, attr) — carve structured derived buffers
      4. grep_buffer(url, ...) + read_buffer(url, ...) — search and read
    ...
    def web_scrape(self, url: str, tag: str, attr: str | list[str] | None = None) -> dict[str, Any]:
        """Extract structured data from an HTML buffer using a tag name and optional attribute specifier."""
        if url not in self._buffers:
            return {"ok": False, "error": f"no buffer named '{url}'. Use web_load_raw to load it first."}
        if not tag.strip():
    ...
    def web_select_css(self, url: str, selector: str, attr: str | list[str] | None = None) -> dict[str, Any]:
        ...
        if not elements:
            return {"ok": True, "found": False, "buffer": url, "selector": selector}
        records = []
