...
class NumPyBufferManager(AgenticObject):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._buffers: dict[str, NumPyBuffer] = {}
    ...
    def list_np_buffers(self) -> dict[str, tuple[int, ...]]:
        ...
        return {name: buf.array.shape for name, buf in self._buffers.items()}
    ...
    def _get_np_buffer(self, name: str) -> np.ndarray | None:
        ...
        if name not in self._buffers:
            return None
        array = self._buffers[name].array
    ...
    def _store_np_buffer(self, name: str, array: np.ndarray) -> None:
        ...
        now = time.time()
        self._buffers[name] = NumPyBuffer(array=array.copy(), created_at=now, modified_at=now)
    ...
    def copy_np_buffer(self, source: str, target: str) -> str:
        ...
        src = self._buffers[source].array
        now = time.time()
        self._buffers[target] = NumPyBuffer(array=src.copy(), created_at=now, modified_at=now)
